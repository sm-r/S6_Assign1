...
public class MedalCnt {
	...
	public static class MyMapper extends Mapper<LongWritable, Text, Text, IntWritable> {
		...
		public void map(LongWritable key, Text value, Context context)
				...
				if (keyvalue[5].equals(swim))
				context.write(new Text(keyvalue[2]), new IntWritable(Integer.parseInt(keyvalue[9])));
//				else
//					context.write(new Text(keyvalue[5]), new IntWritable(Integer.parseInt(keyvalue[9])));
	...
	public static void main(String[] args)  throws Exception {
		...
		Job job = Job.getInstance(conf, "Oly");
		job.setJarByClass(MedalCnt.class);
